...
import { getSelectedScrambleTextTweensCharacters } from '../store/selectors/layer';
...
const EaseEditorScrambleTextCharsInput = (props: EaseEditorScrambleTextCharsInputProps): ReactElement => {
  ...
  const options = [
    ...(charactersValue === 'multi' ? [{ value: 'multi', label: 'multi' }] : []),
    ...DEFAULT_SCRAMBLE_TEXT_TWEEN_CHARACTER_TYPES.map((option) => ({
      value: option,
      label: (() => {
        switch(option) {
          case 'custom':

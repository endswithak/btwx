...
import { setLayersSlowTweenLinearRatio } from '../store/actions/layer';
import { getSelectedSlowTweensLinearRatio } from '../store/selectors/layer';
...
const EaseEditorSlowLinearRatioInput = (props: EaseEditorSlowLinearRatioInputProps): ReactElement => {
  ...
  const linearRatio = useSelector((state: RootState) => getSelectedSlowTweensLinearRatio(state));
  ...
  const handleSubmitSuccess = (newLinearRatio: any): void => {
    ...
    dispatch(setLayersSlowTweenLinearRatio({
      tweens: selectedTweens,
      linearRatio: newLinearRatio
    }));
  }

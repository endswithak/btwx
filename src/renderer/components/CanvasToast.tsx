...
const CanvasToast = (): ReactElement => {
  ...
  const isEnabled = useSelector((state: RootState) => state.canvasSettings.drawing || state.canvasSettings.dragging || state.canvasSettings.selecting || state.canvasSettings.resizing || state.canvasSettings.translating || state.canvasSettings.zooming || state.canvasSettings.measuring);
  const text = useSelector((state: RootState) => (() => {
    if (state.canvasSettings.drawing) {
      ...
      return 'Scrolling';
    } else if (state.canvasSettings.measuring) {
      return 'Measuring';
    }
  })());

...
const ArtboardPreset = (props: ArtboardPresetProps): ReactElement => {
  ...
  const handleDeviceClick = (): void => {
    const newArtboard = new paperMain.Path.Rectangle({
      from: new paperMain.Point(paperMain.view.center.x - (artboardWidth / 2), paperMain.view.center.y - (artboardHeight / 2)).round(),
      to: new paperMain.Point(paperMain.view.center.x + (artboardWidth / 2), paperMain.view.center.y + (artboardHeight / 2)).round(),
      insert: false
    });

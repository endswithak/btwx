...
import { KeyBindingsState, initialMacState as initialMacKeyBindingsState, initialWindowsState as initialWindowsKeyBindingsState } from '../renderer/store/reducers/keyBindings';
...
import { resolveHtmlPath, buildContentMenuTemplate } from './utils';
...
export const createInstance = async ({
      ...
      byId: {
        ...
        [INSTANCE_ID]: {
          ...
          document: new BrowserWindow({
            ...
            title: initialState && initialState.documentSettings ? initialState.documentSettings.name : 'Untitled',
            // icon: `${RESOURCES_PATH}/icons/32x32.png`,
          ...
          preview: new BrowserWindow({
            ...
            title: initialState && initialState.documentSettings ? `${initialState.documentSettings.name} - Preview` : 'Untitled - Preview',
            // icon: `${RESOURCES_PATH}/icons/32x32.png`,
            backgroundColor: '#00000000',
            vibrancy: 'sidebar',
            transparent: true,
  ...
  instance.document.loadURL(resolveHtmlPath('index.html'));
  instance.preview.loadURL(resolveHtmlPath('index.html'));
...
export const createPreferences = async () => {
  ...
  btwxElectron.preferences.loadURL(resolveHtmlPath('index.html'));
...
ipcMain.on('setDocumentTimelineGuidePosition', (event, args) => {
  ...
  const instance = btwxElectron.instance.byId[instanceId];
  instance.document.webContents.executeJavaScript(`setDocumentTimelineGuidePosition(${JSON.stringify({
    time: time
  })})`);
});
...
ipcMain.on('resetPreviewTweeningEvent', (event, args) => {
  ...
  const instance = btwxElectron.instance.byId[instanceId];
  instance.preview.webContents.executeJavaScript(`resetTweeningEvent(${JSON.stringify({
    eventId: eventId
  })})`);
});
